...
GtkWidget* newSpectrumDisplay() {
    ...
    spectrumLow=-sampleRate/2/zoom;
    spectrumHigh=+sampleRate/2/zoom;
...
void drawSpectrum(int height) {
    ...
    minDisplay=f=frequencyA-(sampleRate/2/zoom);
    maxDisplay=frequencyA+(sampleRate/2/zoom);
    
    hzPerPixel=(float)sampleRate/(float)spectrumWIDTH/(float)zoom;
    ...
    if(spectrum->window) {
        ...
        if(subrx) {
            filterLeftX=(filterLow*zoom+(subrxFrequency-frequencyA)-spectrumLow)*spectrumWIDTH/(spectrumHigh-spectrumLow);
            filterRightX=(filterHigh*zoom+(subrxFrequency-frequencyA)-spectrumLow)*spectrumWIDTH/(spectrumHigh-spectrumLow);
        ...
        // draw the filter 
        filterLeftX=(filterLow*zoom-spectrumLow)*spectrumWIDTH/(spectrumHigh-spectrumLow);
        filterRightX=(filterHigh*zoom-spectrumLow)*spectrumWIDTH/(spectrumHigh-spectrumLow);
        if(filterLeftX==filterRightX) filterRightX++;
        ...
        // draw the CW cursor
        gdk_gc_set_rgb_fg_color(gc,&yellow);
	if(mode==modeCWL){
        gdk_draw_line(spectrumPixmap,gc,spectrumWIDTH/2-(cwPitch/hzPerPixel),0,spectrumWIDTH/2-(cwPitch/hzPerPixel),height);
        }; 
	if(mode==modeCWU){
        gdk_draw_line(spectrumPixmap,gc,spectrumWIDTH/2+(cwPitch/hzPerPixel),0,spectrumWIDTH/2+(cwPitch/hzPerPixel),height);
        }; 
...
void plotSpectrum(float* samples,int height) {
    ...
    f=frequencyA-(sampleRate/2/zoom);
    hzPerPixel=(float)sampleRate/(float)spectrumWIDTH/(float)zoom;
    ...
    start_sample_index=(SPECTRUM_SAMPLES>>1)+((spectrumLow*SPECTRUM_SAMPLES)/sampleRate/zoom);
    num_samples=(int)((spectrumHigh-spectrumLow)*SPECTRUM_SAMPLES/sampleRate/zoom);
    ...
    for(i=0; i<spectrumWIDTH; i++) {
        ...
        if(f>0) {
            ...
            //// Soundcard entfernt festen Wert für HPSDR -48.0f
            max = max + -48.0f + preampOffset;
        } else {
            max=-250;
        }
