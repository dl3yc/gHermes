...
void updateVfoADisplay() {
    ...
    if(vfoAFrequency->window) {
        ...
        if (aTransmitting) {
         if (control_1E[1]){
        sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 24'>% 7lld.%03lld.%03lld </span>","#FF00FF",ddsAPlusRit/1000000LL,(ddsAPlusRit%1000000LL)/1000LL,ddsAPlusRit%1000LL);
         } else {
        sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 24'>% 7lld.%03lld.%03lld </span>","#FF0000",frequencyA/1000000LL,(frequencyA%1000000LL)/1000LL,frequencyA%1000LL);
         }
        } else {
         if (control_1E[1]){
          freqAPitch = frequencyA;
          if(mode==modeCWL) {freqAPitch = frequencyA - cwPitch;};  
          if(mode==modeCWU) {freqAPitch = frequencyA + cwPitch;};  
         sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 24'>% 7lld.%03lld.%03lld </span>","#FFFF00",freqAPitch/1000000LL,(freqAPitch%1000000LL)/1000LL,freqAPitch%1000LL);
         } else {
         sprintf(temp,"<span foreground='%s' background='#2C2C2C' font_desc='Sans Bold 24'>% 7lld.%03lld.%03lld </span>","#00FF00",frequencyA/1000000LL,(frequencyA%1000000LL)/1000LL,frequencyA%1000LL);
         }
        };
...
void setAFrequency(long long f) {
    ...
    ddsAPlusRit = ddsAFrequency;

    if(mode==modeCWL) {ddsAPlusRit = ddsAFrequency - (long long) cwPitch;};
    if(mode==modeCWU) {ddsAPlusRit = ddsAFrequency + (long long) cwPitch;};
...
GtkWidget* buildVfoUI() {
    ...
    gtk_widget_modify_bg(vfoAFrame, GTK_STATE_NORMAL,&background);
    gtk_widget_modify_fg(label, GTK_STATE_NORMAL, &white);
    ...
    gtk_widget_modify_bg(vfoBFrame, GTK_STATE_NORMAL,&background);
    gtk_widget_modify_fg(label, GTK_STATE_NORMAL, &white);
#ifdef NETBOOK
