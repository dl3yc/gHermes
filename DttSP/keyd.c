...
CWtoneExchange (float *bufl, float *bufr, int nframes)
{
	...
	if (cw_ring_reset)
	{
//fprintf(stderr,"CWtoneExchange: cw_ring_reset\n");
	...
	{
//fprintf(stderr,"CWtoneExchange: cw_ring_reset=TRUE nframes=%d numsamps=%d\n",nframes,numsamps);
		memset (bufl, 0, bytesize);
		memset (bufr, 0, bytesize);
		cw_ring_reset = TRUE;
	}
	...
	{
//fprintf(stderr,"CWtoneExchange: nframes=%d\n",nframes);
		EnterCriticalSection (cs_cw);
...
send_tone (void)
{  
//fprintf(stderr,"send_tone\n");
	if (ringb_float_write_space (lring) < TONE_SIZE)
	{
		cw_ring_reset = TRUE;
//fprintf(stderr,"send_tone: cw_ring_reset=TRUE\n");
	}
	else
	{
		int i;
...
send_silence (void)
{
//fprintf(stderr,"send_silence\n");
	if (ringb_float_write_space (lring) < TONE_SIZE)
	{
		cw_ring_reset = TRUE;
//fprintf(stderr,"send_silence: cw_ring_reset=TRUE\n");
	}
	else
	{
		int i;
...
key_thread_process (REAL del, BOOLEAN dash, BOOLEAN dot, BOOLEAN keyprog)
{
	BOOLEAN keydown;
	extern BOOLEAN read_straight_key (KeyerState ks, BOOLEAN keyed);
//fprintf(stderr,"key_thread_process dash=%d dot=%d\n",dash,dot);
...
SetKeyerResetSize (unsigned int sizer)
{
	SIZEBUF = sizer;
	cw_ring_reset = TRUE;
//fprintf(stderr,"SetKeyerResetSize: cw_ring_reset=TRUE\n");
}
...
//fprintf(stderr,"SetKeyerSampleRate\n");
	if (HiPerformance)
	{
		key_poll_period = 1;
		TONE_SIZE = 48 * factor;
