...
void twoToneButtonCallback(GtkWidget* widget,gpointer data) {
    if(GTK_TOGGLE_BUTTON(widget)->active) {
        setTwoTone(1);
    } else {
        setTwoTone(0);
...
GtkWidget* hpsdrSetupUI() {
    ...
    box=gtk_hbox_new(FALSE,3);
    label=gtk_label_new("Transmit Tune Tone:		");
    gtk_widget_show(label);
    gtk_box_pack_start(GTK_BOX(box),label,FALSE,FALSE,2);
    twoToneWidget=gtk_check_button_new_with_label("TwoTone");
    gtk_widget_show(twoToneWidget);
    gtk_box_pack_start(GTK_BOX(box),twoToneWidget,FALSE,FALSE,2);
    g_signal_connect(G_OBJECT(twoToneWidget),"clicked",G_CALLBACK(twoToneButtonCallback),NULL);
    gtk_widget_show(box);
    gtk_box_pack_start(GTK_BOX(hpsdrPage),box,FALSE,FALSE,2);

    box=gtk_hbox_new(FALSE,3);
    label=gtk_label_new("Mic Boost PreAmpl:		");
    gtk_widget_show(label);
    gtk_box_pack_start(GTK_BOX(box),label,FALSE,FALSE,2);
    micBoostWidget=gtk_check_button_new_with_label("20 dB Boost");
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(twoToneWidget),twotone);
